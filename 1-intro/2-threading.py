...
def make_burger(student_id):
    ...
    print(f"[{end_time.strftime('%H:%M:%S')}] เสร็จแล้ว! เบอร์เกอร์ของนักเรียนคนที่ {student_id} เริ่มทำตอน {start_time.strftime('%H:%M:%S')} และเสร็จตอน {end_time.strftime('%H:%M:%S')} ใช้เวลา {duration:.2f} วินาที")
